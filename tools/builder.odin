...
// CIE XYZ -> Linear LMS (cone response) matrix,
// as defined by Björn Ottosson for the OKLab color space.
...
rgb_to_xyz :: #force_inline proc "contextless" (p: linalg.Matrix3f64) -> linalg.Matrix3f64 {
	// Convert each primary xyY chromaticity (row of p) into an XYZ vector, via X = x / y, Y = 1, Z = (1 - x - y) / y.
	r: CIE_XYZ = {p[0, 0], p[0, 1], p[0, 2]} / p[0, 1]
	g: CIE_XYZ = {p[1, 0], p[1, 1], p[1, 2]} / p[1, 1]
	b: CIE_XYZ = {p[2, 0], p[2, 1], p[2, 2]} / p[2, 1]

	// Pack the three primary XYZ vectors as columns: unscaled RGB to XYZ matrix.
	...
	// Convert the whitepoint chromaticity into an XYZ vector the same way.
	w: CIE_XYZ = CIE_XYZ{D65.x, D65.y, 1.0 - D65.x - D65.y} / D65.y

	// Solve m * s = w for per-channel scale factors, so that RGB (1, 1, 1) maps exactly onto the whitepoint.
	s: CIE_XYZ = linalg.mul(linalg.inverse(m), w)

	// Place the scale factors on a diagonal matrix, to scale columns of m.
	...
	// Apply the scale factors to columns of m: the final RGB to XYZ matrix.
	return linalg.mul(m, diag_s)
}

@(require_results)
rgb_to_lms :: #force_inline proc "contextless" (m: linalg.Matrix3f64) -> linalg.Matrix3f64 {
	// Chain the RGB to XYZ matrix with the fixed XYZ to LMS matrix.
...
main :: proc() {
	...
	fmt.println("// GENERATED BY THE BUILDER TOOL; DO NOT EDIT BY HAND.\n")
